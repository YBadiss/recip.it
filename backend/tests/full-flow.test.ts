...
describe('Full Recipe Import Flow E2E Tests', () => {
  ...
  beforeEach(async () => {
    ...
    await testServer.setupTestUser();
  });
  ...
  it('should import a recipe from URL via the API', async () => {
    ...
    const recipeStore = testServer.getRecipeStore();
  ...
  it('should retrieve all recipes via the API', async () => {
    ...
    const recipeStore = testServer.getRecipeStore();
    const userStore = testServer.getUserStore();
    const testUserId = testServer.getTestUserId();
    ...
    await userStore.addRecipeToUser(testUserId, id1);

    // Wait a bit to ensure different timestamps
    await new Promise(resolve => setTimeout(resolve, 100));

    const id2 = await recipeStore.createRecipe(recipe2);
    await userStore.addRecipeToUser(testUserId, id2);
    ...
    // Check for inUserList flag
    expect(recipe1Response.inUserList).toBe(true);
    expect(recipe2Response.inUserList).toBe(true);
  });
...
function createMockRecipeExtractor(): RecipeExtractor {
  ...
  mockExtractor.extractRecipeFromUrl = jest.fn(async () => ({
    ...
    materials: [{ id: 'mat1', name: 'URL Material 1' }],
    steps: [
